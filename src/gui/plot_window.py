...
class PlotWindow(QMainWindow):
    ...
    def plot_data(self, measurements):
        ...
        if valid_measurements:
            ...
            E_i_values = [abs(m.delta_E_i) for m in valid_measurements]  # Keep in Joules
            E_o_values = [abs(m.delta_E_o) for m in valid_measurements]  # Keep in Joules
            ...
            if len(B_values) > 1:
                ...
                self.ax.plot(B_line, p_i(B_line), 'b-', linewidth=2, 
                            label=f'Inner fit: {z_i[0]:.3e} J/T', alpha=0.7, zorder=2)
                
                # Outer trend line
                z_o = np.polyfit(B_array, E_o_array, 1)
                p_o = np.poly1d(z_o)
                self.ax.plot(B_line, p_o(B_line), 'r-', linewidth=2, 
                            label=f'Outer fit: {z_o[0]:.3e} J/T', alpha=0.7, zorder=2)
